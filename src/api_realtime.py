...
try:
    # LAZY IMPORT (fix circular import)
    run_scrape = start_scheduler = stop_scheduler = set_broadcast = None
    HASHTAGS = []

    def _load_apify():
        global run_scrape, start_scheduler, stop_scheduler, set_broadcast, HASHTAGS
        from src.apify_scraper import (
            run_scrape, start_scheduler, stop_scheduler, set_broadcast, HASHTAGS
        )

    APIFY_AVAILABLE = True
    print("apify_scraper lazy-loaded")

except Exception as e:
    print(f"apify_scraper unavailable: {e}")
