...
Run: uvicorn src.api_realtime:app --host 0.0.0.0 --port 8000 --reload
"""
...
async def startup_event():
    global yolo_model, eff_model, lr_model, vectorizer, sentiment_model
    print("="*55)
    ...
    # Auto-rescore existing posts on every startup to fix stale labels in DB
    if DB_AVAILABLE:
        try:
            fixed = rescore_existing_posts()
            print(f"Startup rescore: fixed {fixed} posts")
            # Trigger alerts for any fake posts found after rescoring
            if ALERTS_AVAILABLE:
...
# High-confidence fake phrases that alone warrant a strong signal
...
# Suspicious combos: score boosted when these co-occur
SUSPICIOUS_COMBOS = [
    ({"dm", "interested"},       0.35),
    ({"dm", "price"},            0.40),
    ({"dm", "buy"},              0.40),
...
def get_text_score(text):
    ...
    # No ML model — rely solely on keyword score
    return round(kw, 4)
...
def _derive_label(post: dict) -> str:
    ...
    try:
        ...
        # Inline fallback using shared thresholds if DB import failed
...
def feed_endpoint(limit:int=50, offset:int=0, source:Optional[str]=None):
    try:
        if DB_AVAILABLE:
            ...
            for p in posts:
                # If score is zero, enrich with ML score then let _derive_score re-label
                if not p.get("final_score") and p.get("caption"):
                    try:
                        p["final_score"] = get_text_score(p["caption"])
                    except Exception:
                        p["final_score"] = 0.0
                # Always re-derive label via the single source of truth
...
def rescore_endpoint():
    ...
    try:
        ...
        # Trigger alerts for newly-labeled fake posts
...
def sentiment_endpoint(req: SentimentRequest):
    ...
    # Use shared thresholds — consistent with _derive_label and apify_scraper
    if text_score >= THRESHOLD_FAKE:
        label = "fake"
    elif text_score >= THRESHOLD_UNCERTAIN:
        label = "uncertain"
    else:
        # Fall back to sentiment model label when score is low
        label = sent["label"]

    counterfeit_pct = round(text_score * 100, 1)
    # Scale display bars so they always sum to ~100 and counterfeit dominates when score is high
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("src.api_realtime:app", host="0.0.0.0", port=8000, reload=True)